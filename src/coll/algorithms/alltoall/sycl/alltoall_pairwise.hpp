...
ccl::event alltoall_sycl_numa_pairwise_rdma_oneshot_split(sycl::queue& q,
    ...
    //fprintf(stderr, "alltoall_sycl_numa_pairwise_rdma_oneshot_split[%d] numa_size:%d\n", rank,numa_size);
    /* scale up */
    if (numa_comm->size() > 1 && 1) {
        int first_numa_rank = numa_comm->get_global_rank(0);
        bool use_numa = true;
        if (split_numa > 0) {
            first_numa_rank = first_node_rank;
            use_numa = false;
        }
                ...
                for (int i = numa_size; i < world; i++) {
                    ...
                    if (numa_comm->rank() < split_numa) {
                        if (src % numa_size < split_numa) {
                           // sycl::ext::oneapi::experimental::printf("not rev rank[%d] numa_comm->rank[%d] split_numa[%d] src[%d]\n", rank,numa_comm->rank(),split_numa,src);
                            continue;
                        }
                    }
                    int64_t recv_tag =

#ifndef __ALLTOALL_LL256_HPP__
#define __ALLTOALL_LL256_HPP__
...
sycl::event arc_ll256_alltoall_sync(const void *src,
                                void *dst,
                                size_t count,
                                ccl::datatype dtype,
                                ccl_comm *comm,
...
sycl::event arc_ll256_alltoall_onestep(const void *src,
                                void *dst,
                int dst_rank,
                int step,
                                size_t count,
...
void arc_alltoall_init(const void *src,
                         void *dst,
                         size_t count,
                         ccl::datatype dtype,
                         ccl_comm *comm,
    ...
    if (send_buf != recv_buf)
        q.memcpy(recv_buf, send_buf, dt_sz * count * comm_size);

    coll_init(comm, global_stream);
}

sycl::event arc_alltoall_onestep(const void *src,
                         void *dst,
                         int dst_rank,
                         int step,
                         size_t count,
                         ...
                         ccl_comm *comm,
                         ccl_stream *global_stream) {
    return arc_ll256_alltoall_onestep(src, dst,dst_rank,step, count, dtype, comm, global_stream);
}
...
#else
    sycl::event e;
    arc_alltoall_init(src, dst, count, dtype, comm, global_stream);
    std::shared_ptr<ccl_comm> node_comm = comm->get_node_comm();
    const int comm_size = node_comm->size();
    const int comm_rank = node_comm->rank();
    sycl::queue q = global_stream->get_native_stream();
    for (int k = 1; k < comm_size; k++) {
        int next_rank = comm_rank ^ k;
        e = arc_alltoall_onestep(src, dst,next_rank,k, count, dtype, comm, global_stream);
    }
    return ccl::event::create_from_native(e);
#endif
}

#endif // __ALLTOALL_LL256_HPP__
